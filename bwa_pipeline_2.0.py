...
run_dir = "/proj/omics4tb/sturkarslan/syntrophy_raw_sequences/1000-gen"
...
def runBWA(alignment_results,file_ext,first_file_name,second_file_name,lane,folder_name,sample_id,RGId, RGSm, RGLb, RGPu,files_2_delete):
    ...
    # bwa run command
    cmd = "bwa mem -t 16 -R '%s' %s %s %s > %s.sam" %(read_group, genome_fasta, first_pair_trimmed, second_pair_trimmed,base_file_name)
...
def gatk_variants(alignment_files_path,gatk_results,exp_name,folder_name): #with GATK HaploTypeCaller
    ...
    # haplotype command
    cmd1 = '%s -Xmx8G -jar %s -T HaplotypeCaller -R %s -I %s_marked.bam --genotyping_mode DISCOVERY -ploidy 1 -stand_emit_conf 30 -stand_call_conf 30 -o %s_gatk_raw.vcf' %(javaPath, gatkPath, genome_fasta, alignment_files_path, gatk_files_path)
    # Select snp variants
    cmd2 = '%s -Xmx8G -jar %s -T SelectVariants -R %s -V %s_gatk_raw.vcf -selectType SNP -o %s_gatk_snps.vcf' %(javaPath, gatkPath, genome_fasta, gatk_files_path,gatk_files_path)
    # Apply filters to SNPs
    cmd3 = "%s -Xmx8G -jar %s -T VariantFiltration -R %s -V %s_gatk_snps.vcf --filterExpression 'QD < 2.0 || FS > 60.0 || MQ < 40.0 || MQRankSum < -12.5 || ReadPosRankSum < -8.0' --filterName 'my_snp_filter' -o %s_gatk_snps_filtered.vcf" %(javaPath, gatkPath, genome_fasta, gatk_files_path, gatk_files_path)
    # Select indel variants
    cmd4 = '%s -Xmx8G -jar %s -T SelectVariants -R %s -V %s_gatk_raw.vcf -selectType INDEL -o %s_gatk_inds.vcf' %(javaPath, gatkPath, genome_fasta, gatk_files_path,gatk_files_path)
    # Apply filters to indels
    cmd5 = "%s -Xmx8G -jar %s -T VariantFiltration -R %s -V %s_gatk_inds.vcf --filterExpression 'QD < 2.0 || FS > 200.0 || ReadPosRankSum < -20.0' --filterName 'my_indel_filter' -o %s_gatk_inds_filtered.vcf" %(javaPath, gatkPath, genome_fasta, gatk_files_path, gatk_files_path)
    # Merge vcf files
    cmd6 = "%s -Xmx8G -jar %s -T CombineVariants -R %s --variant %s_gatk_snps_filtered.vcf --variant %s_gatk_inds_filtered.vcf -o %s_gatk_final.vcf  -genotypeMergeOptions UNSORTED" %(javaPath, gatkPath, genome_fasta, gatk_files_path, gatk_files_path, gatk_files_path)
...
def combine_variants(snpeff_filtered_vcf,snpeff_final,combined_variants):
    ...
    for line in f:
        ...
        if vcf_file_program == "varscan_results":
            ...
            #print(my_fields)
            # grab DP4 field
            dp4 = filter(lambda x:'DP4' in x, my_fields)[0]
            #print dp4
...
def run_pipeline():
    ...
    for data_folder in data_folders:
        ...
        first_pair_files = glob.glob('%s/*_R1_*.fastq*' %(data_folder))
        second_pair_files = glob.glob('%s/_R2_*.fastq*' %(data_folder))
        ...
        for first_pair_file in first_pair_files:
            ...
            second_pair_file = first_pair_file.replace('_R1_', '_R2_')
